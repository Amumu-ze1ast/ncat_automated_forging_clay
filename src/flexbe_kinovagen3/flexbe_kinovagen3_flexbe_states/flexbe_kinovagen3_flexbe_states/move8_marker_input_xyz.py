...
class Move8MoveToMarkerState(EventState):
    """
    FlexBE state to move robot to detected marker position with offset and orientation.
    
    Uses input keys for XYZ position (e.g., from ArUco detection) and parameters for orientation and offset.
    Uses the single_pose_ik_solver C++ TRAC-IK solver to compute joint angles.
    ...
    -- x_offset         float   X-axis offset from marker (final_x = marker_x - x_offset) (meters, default: 0.0)
    ...
    def __init__(self, target_roll=0.0, target_pitch=3.14159, target_yaw=0.0,
                 x_offset=0.0, speed_scale=0.3, tolerance=0.01):
        
        super(Move8MoveToMarkerState, self).__init__(
    ...
    def execute(self, userdata):
        ...
        # Check timeout
        elapsed = Move8MoveToMarkerState._node.get_clock().now() - self._start_time
    ...
    def on_enter(self, userdata):
        ...
        # Apply offset: final_x = marker_x - x_offset
        final_x = marker_x - self._x_offset
        final_y = marker_y
        final_z = marker_z
        
        Logger.loginfo(f'X offset: {self._x_offset:.3f}m (final_x = marker_x - x_offset)')
        Logger.loginfo(f'Final target position: XYZ=({final_x:.3f}, {final_y:.3f}, {final_z:.3f})')
        ...
        joint_solution = self._call_ik_solver(final_x, final_y, final_z)
        ...
        self._start_time = Move8MoveToMarkerState._node.get_clock().now()
    ...
    def _call_ik_solver(self, target_x, target_y, target_z):
        """
        Call the C++ single_pose_ik_solver executable.
