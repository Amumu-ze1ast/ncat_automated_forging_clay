...
class Move6IncrementalRPYMoveState(EventState):
    """
    FlexBE state for incremental RPY (roll-pitch-yaw) rotation while maintaining current XYZ position.
    
    Uses the incremental_rpy_ik_solver C++ executable to compute IK solution.
    
    Parameters:
    -- delta_roll       float   Incremental roll rotation in radians (default: 0.0)
    -- delta_pitch      float   Incremental pitch rotation in radians (default: 0.0)
    -- delta_yaw        float   Incremental yaw rotation in radians (default: 0.0)
    ...
    def __init__(self, delta_roll=0.0, delta_pitch=0.0, delta_yaw=0.0, speed_scale=0.3, tolerance=0.01):
        super(Move6IncrementalRPYMoveState, self).__init__(
    ...
    def execute(self, userdata):
        ...
        if self._target_joints is not None:
            ...
            if np.all(error < self._tolerance):
                Logger.loginfo('Target orientation reached!')
                userdata.final_joints = self._target_joints
                self._return = 'done'
                return 'done'
        
        # Check timeout
        elapsed = Move6IncrementalRPYMoveState._node.get_clock().now() - self._start_time
    ...
    def on_enter(self, userdata):
        ...
        Logger.loginfo(f'Incremental RPY rotation: ΔRoll={self._delta_roll:.3f}, ΔPitch={self._delta_pitch:.3f}, ΔYaw={self._delta_yaw:.3f}')
        Logger.loginfo(f'Speed scale: {self._speed_scale}')
        
        # Call C++ IK solver
        joint_solution = self._call_incremental_rpy_solver()
        ...
        self._start_time = Move6IncrementalRPYMoveState._node.get_clock().now()
        self._send_trajectory(joint_solution)

    def on_exit(self, userdata):
        """Called when leaving the state."""
        Logger.loginfo('IncrementalRPYMove state exiting')

    def on_start(self):
        """Called when behavior starts."""
        Logger.loginfo('IncrementalRPYMove state started')

    def on_stop(self):
        """Called when behavior stops."""
        Logger.loginfo('IncrementalRPYMove state stopped')

    # ==================== Helper Methods ====================

    def _call_incremental_rpy_solver(self):
        """
        Call the C++ incremental_rpy_ik_solver executable.
        Returns joint angle solution or None if failed.
        """
        try:
            # Build command
            cmd = [
                'ros2', 'run', 'trac_ik_examples', 'move4_increment_rpy',
                str(self._delta_roll),
                str(self._delta_pitch),
                str(self._delta_yaw)
            ]
            
            Logger.loginfo('Calling incremental RPY IK solver...')
