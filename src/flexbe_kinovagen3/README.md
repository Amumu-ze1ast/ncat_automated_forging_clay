# FlexBE States and Behaviors for flexbe_kinovagen3
...
This raw repository has several folders and files with the generic name `flexbe_kinovagen3`.
...
and change the relevant `flexbe_kinovagen3` text to `my_new_project_name` as needed.
...
## Example States in `flexbe_kinovagen3_flexbe_states`
...
## Example Behaviors in `flexbe_kinovagen3_flexbe_behaviors`
...
[`copy_behavior`](flexbe_kinovagen3_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run flexbe_kinovagen3_flexbe_behavior copy_behavior` to see the usage guide. 
